  ...
  return (
    <GameFrame bgImage={bg}>
      <header className="pt-6 px-5 flex items-center gap-3">
        ...
        <h1 className="pixel text-glow text-lg text-primary flex-1 text-center pr-4">STORE</h1>
        <div className="pixel text-xs text-primary bg-background/70 border-2 border-primary px-2 py-1">
          ⭐ 240
        </div>
      </header>
      ...
      <main className="flex-1 px-5 py-6 grid grid-cols-2 gap-3 overflow-y-auto">
        {items.map((it) => (
          <div key={it.name} className="pixel-frame p-3 flex flex-col items-center gap-2 bg-card/90">
            ...
            <button className="pixel-btn text-[10px] w-full" style={{ padding: "0.5rem" }}>
              ⭐ {it.price}
            </button>
          </div>
        ))}
